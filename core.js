...
function planPeriod(ym=$('month').value){const[y,m]=String(ym||new Date().toISOString().slice(0,7)).split('-').map(Number),pad=v=>String(v).padStart(2,'0'),fmt=d=>d.getUTCFullYear()+'-'+pad(d.getUTCMonth()+1)+'-'+pad(d.getUTCDate()),start=new Date(Date.UTC(y,m-2,21)),end=new Date(Date.UTC(y,m-1,20));return{start:fmt(start),end:fmt(end)}}
