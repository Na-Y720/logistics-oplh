...
function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}function monthDate(){return ($('month').value||companyMonthToday())+'-01'}
function planPeriod(ym=$('month').value){const[y,m]=String(ym||companyMonthToday()).split('-').map(Number),pad=v=>String(v).padStart(2,'0'),fmt=d=>d.getUTCFullYear()+'-'+pad(d.getUTCMonth()+1)+'-'+pad(d.getUTCDate()),start=new Date(Date.UTC(y,m-2,21)),end=new Date(Date.UTC(y,m-1,20));return{start:fmt(start),end:fmt(end)}}
function staffVisibleInMonth(s,ym=$('month').value){if(!s?.retirement_date)return true;return String(s.retirement_date).slice(0,10)>=planPeriod(ym).start}
function retirementInMonth(s,ym=$('month').value){if(!s?.retirement_date)return false;const d=String(s.retirement_date).slice(0,10),p=planPeriod(ym);return d>=p.start&&d<=p.end}
function retirementText(s){if(!s?.retirement_date)return'';const d=String(s.retirement_date).slice(0,10);return '退職 '+Number(d.slice(5,7))+'/'+Number(d.slice(8,10))}
...
async function loadMonth(){
 ...
 try{
  ...
  monthRows=(rows||[]).map(r=>({...r,staff:staff.find(s=>s.id===r.staff_id),default:defaults.find(d=>d.staff_id===r.staff_id)})).filter(r=>r.staff&&staffVisibleInMonth(r.staff,$('month').value));
...
function renderMonthly(){
 ...
 $('requiredHours').textContent=Number.isFinite(required)?fmt(required)+'h':'—';$('securedHours').textContent=fmt(secured)+'h';$('gapHours').textContent=Number.isFinite(gap)?(gap>=0?'+':'')+fmt(gap)+'h':'—';$('requiredOplh').textContent=Number.isFinite(need)?fmt(need,2):'—';$('staffCount').textContent=monthRows.length+'人';$('includedCount').textContent=included+'人';$('totalDays').textContent=fmt(days)+'日';$('grossHours').textContent=fmt(gross)+'h';$('shipHours').textContent=fmt(secured)+'h';$('otherHours').textContent=fmt(Math.max(0,gross-secured))+'h';$('fulfillRate').textContent=Number.isFinite(ful)?fmt(ful)+'%':'—';$('fulfillBar').style.width=Math.max(0,Math.min(100,Number.isFinite(ful)?ful:0))+'%';
